...
#correct, here is another way
#shcool_name_finder <- md_payments |> filter(str_detect(vendor_name, "school")) |> group_by(vendor_name) |> summarize(count = n())
...
A2: 2021 had the largest total amount. What happened between 2013 and 2020 since all the years between them are jumbled.
...
A3. You would use get_dupes() to clean out the duplicate names. Some examples are Whiting Turner, Contracting Company vs Contracting CO, Plano Coudon vs Plano Condon, and Olympus America LLC. and Olympus America LLC without a (.). I would then us case_when and decide on whichever version of the name is better and change all similar ones to the now 'correct' name.

```{r}
payments |> filter(str_detect(agency_name, "CAPITAL PROJECTS- UNIVERSITY OF MARYLAND")) |>
  group_by(vendor_name) |>
  summarise(total_amount = sum(amount)) |>
  arrange(desc(total_amount))

#correct, here is another way
#capital_projects_finder <- md_payments |> filter(str_detect(agency_name, "CAPITAL PROJECTS")) |> group_by(agency_name) |> summarize(count = n())
```
...
A4: It has technically been steadily decreasing since 2012 where it reached it highest amount of money going in. It then went down to an average of around 6-7, going as low as 4 in 2021 but bouncing back to the 6-7 average in 2022.
...
#md_grants_loans |>
 #mutate(description = str_to_upper (description)) |>
 #filter(str_detect(description, "STEMCELL RESEARCH GRANT")) |>
