...
```{r}
# load the data
md_absentee_ballots <- read_csv("md_absentee_ballots.csv")|> clean_names()
md_absentee_ballots_date <- read_csv("md_absentee_ballots_date.csv")|> clean_names()
```
...
**A2. St. Mary's, Harford, Washington, Worcester, Carroll, Queen Anne's, Caroline, Cecil, Allegany, and Garrett.**
...
**A3 Democrats with the most, then republicans, and then other. I would say it does match up.**
...
**A4. I see 5 counties that are out of the average range and everyone else is somewhat the same. Carroll looks like it could be considered under-performing while Baltimore County, PG, and Montgomery seem to be over-performing.**
