...
maggies_response <- axolotr::ask(
  prompt = paste("Given the following text, extract information into a CSV file with the following structure, including headers, with no yapping and do not include the first sentence of the response: title, year, role, director, role, co-star and awards", text),
  model = "llama-3.1-8b-instant"
)
maggies_response <- read_csv(maggies_response)
```

**Answer: So the answer is correct and it only grabbed information from the article. So even though she was in many more Harry Potter Movies.**
...
course_response <- axolotr::ask(
  prompt = paste("can you explain without yapping: what the r_code_example does to the umd_courses csv file", r_code_example),
  model = "llama-3.1-8b-instant"
)
course_response
...
**Answer: It said it was correct and well structured. When I ran it the first time it said there were ways to improve it and potential errors that could occur.**
...
**Answer: It gives step by step instructions on how you would go about doing that as well as different patterns you might find. The only thing it does not explain is the significance of the data.**
...
**Answer: Although it is a news article, it is not something real that happened and it uses fake quotes. But in terms of generating a good-sounding piece, it fulfilled the requirement.**
