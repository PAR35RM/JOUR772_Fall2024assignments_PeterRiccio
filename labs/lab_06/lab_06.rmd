...
baltimore_911 <- read.csv("data/baltimore_911.csv")|> clean_names()
...
baltimore_911_with_maryland_zcta <- maryland_zcta |> right_join(baltimore_911, join_by(zcta5n == zip_code))

county_combined <- bind_rows(list(allegany_911, cecil_911, carroll_911))
```
...
A2. The location that had the most was 314 Grove Neck Drive in Earleville which is a part of Cecil county.
...
A3. some of the areas are 21201, 21202, and 21205
...
A4: I though it would be cool to see the total number of calls by district and zip code as some districts have more than one code. The answer was WD (21217) with 489 calls

```{r}
baltimore_911_with_maryland_zcta|>
  group_by(zcta5n, district)|>
  summarise(total = n())|>
  arrange(desc(total))
```
